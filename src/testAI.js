...
var TestAI = {
  ...
  create: function(){
    ...
    text = game.add.text(screen1Width*.02,screen1Height*.09, 'Level: '+level, {fontSize: '20px', fill: '#000000'});
  ...
  update: function(){
    ...
    if (squirrelAlive){
      ...
      if ((level > 2)&&(paraCurr < paraLength)){
        currFeatures = player.stateToFeatures();
        isDiving = 3;
        squirrel.parachute();
        paraCurr += 1;
        framesDiving += 1;
      } else if  ((level > 1)&&(boostCurr < boostLength)){
        currFeatures = player.stateToFeatures();
        isDiving = 2;
        boostCurr += 1;
      ...
      if (squirrelX > levelLength){
        ...
        squirrelProgress.destroy();
        game.state.start('testAIComplete');
